...
float manualOffset = 0.045;                  // Key in value to manually offset the initial value
...
// For example, a 30A Hall-Effect Current Transformer rated at 50A, 2.5V +/- 0.625V, mVperAmp will be 625 mV / 30A = 20.83333 mV/A
float supplyVoltage = 3300;                 // Analog input pin maximum supply voltage, Arduino Uno or Mega is 5000mV while Arduino due is 3300mV
...
void loop()                                                                                                   /*codes to run again and again */
{
  ...
  if (currentSampleCount == 4000)                                                                       /* after 4000 count or 800 milli seconds (0.8 second), do this following codes*/
  {
    ...
    if ((FinalRMSCurrent <= (625 / mVperAmpValue / 100)) or  (FinalRMSCurrent >30))                                         /* if the current detected is less than or up to 1%, set current value to 0A (0,3)*/
    {
      FinalRMSCurrent = 0;
    }
