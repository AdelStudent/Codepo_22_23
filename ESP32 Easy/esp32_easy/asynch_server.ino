...
void verify_time(){
    //This function will verify if the time is correct
    //If not, it will ask the time to the internet
}
void receive_message(){
    //This function will check if the serial port has received a message
    if(Serial.available()){
        String data = Serial1.readStringUntil('\n');
        if(data = "What time is it?"){
            String time = "07/04/23 12:00";
            //String time = ask_hour_to_internet();
