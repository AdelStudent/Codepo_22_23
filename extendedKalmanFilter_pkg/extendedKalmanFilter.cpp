  ...
  double DeltaT, double Qn_rated)
{
  ...
  /*  Linearizing the model by linearizing the C matrix */
    //-> Linearisation d'une matrice = Jacobian autour d'un point d'equilibre 
  dOCV = SOC * (SOC * (SOC * dSOCOCV[0] + dSOCOCV[1]) + dSOCOCV[2]) + dSOCOCV[3];

  /*  dUoc(SOC)/dSOC at SOC = SOCk */
  if (X[0] > 1.0) { //Si le SOC est de 100%
    dOCV = 0.0; //L'open circuit voltage ne change plus 
  }

  /*  Hx : Jacobian measurement matrix of x */
  dv[1] = 0.0; //La seconde valeur vaut 0
  d = DeltaT / (Z[2] * Z[2]);
  dv[3] = -b_I * (exp(d) - 1.0);
